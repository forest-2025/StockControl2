...
/* 	jQueryを使用している.
	...
	→  ブラウザがjquery.jsを自動リクエストする. → jquery.jsレスポンスを受け取ったらブラウザが読み込んで,$.ajax() が呼ばれる
	→ jQuery が内部で XMLHttpRequest（XHR）オブジェクト を作る. → jQuery はさらにその XHR をラップして jqXHR オブジェクト を返す.
	→ この jqXHR オブジェクトに .done(), .fail(), .always() のコールバック関数が登録される.
	→ 準備が整いjQueryが使用できる. → 画像表示ボタンをクリックされたら,URLのところのパスにGETリクエストする(jqXHR オブジェクトに進行中・成功・失敗など,
	現在の通信の進行状態が入る). → コントローラで画像を探しHTTPステータスと画像があれば,	その情報をbodyに入れて返す. 
	→ その結果をうけて,jqXHR オブジェクトに HTTPステータスやヘッダー情報がはいる(レスポンスのデータは基本的に保持していない).
	→ jsの処理が実行される(コールバック関数に,レスポンスのデータ(ここではblob)・textStatus(通信結果を文字列で表したString型の変数),
	jqXHR (HTTPステータスやヘッダー情報)・エラーなら errorThrown (通信が失敗したときに,エラーの内容を文字列で表したString型の変数)が渡される).
	...
	Ajaxとは画面表示中にウェブページ全体を再読み込みせずに,jsが裏でサーバーと非同期で通信し必要なデータだけ受け取り画面の一部だけ更新する技術のこと.
	...
	.done()は jQuery の非同期処理（主に $.ajax()）が成功したときに実行したい処理を登録するための関数。
	jsが読み込まれて .done で実行する処理(function(blob, _, jqXHR){} のこと.コールバック関数という.)を登録するだけで,
	実行されるのは非同期処理が成功したとき.
	本来は .done(function(blob, textStatus, jqXHR) だが, textStatus を _ にしている.
	これは,この関数内でステータス文字列を使わない（無視する）という慣習的な記述方法.
	
	.fail(function){}) は非同期処理が失敗したときのコールバック関数を登録する関数.
	.fail(function(jqXHR,textStatus,errorThrown){}) のように引数があるのが一般的だが,
	失敗したことがわかれば詳細なエラー内容は必要ないので(表示したりしないので)引数無しにしている.
	...
	ここでその答えを待たずにconsole.log("D");が実行され,レスポンスが返れば該当する処理(done,fail)が行われるということ.
*/
