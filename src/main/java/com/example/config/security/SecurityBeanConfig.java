...
AuthenticationFilter	リクエストがログイン処理かどうかを判断して,ログイン処理ならフォームに入力された username ・ password を取得する.
						...
						Authentication 型の変数に代入する(このとき実装クラスのフィールドauthenticatedはfalse（未認証）で作成される).
						このクラスはデフォルトのため,社員ID ・ username ・ password(パスワード)でログインなどには対応できない)
						で作成されたオブジェクトを AuthenticationManager に渡す.
  ↓
...
AuthenticationProvider	ここでBean登録されているのはここの部分のこと.これはアプリケーション起動時に Bean 登録される内容のこと.
						...
						複数設定できる.例えばメールアドレスとパスワードでログインする方法,ログインIDとパスワードでログインする方法など使い分けができる.
						...
						リクエストがきて AuthenticationManager から認証を任されると,authenticate()メソッドが呼ばれる.
...
認証成功 or 失敗			認証成功 → authenticated=true の Authentication オブジェクトを返す.
						...
						このオブジェクトは SecurityContextPersistenceFilter が HTTPセッションに保存する.
						リクエストが来ると SecurityContextPersistenceFilter が呼ばれて, HTTPセッションから SecurityContext を復元,
						または新規作成(未ログインでもリクエストが来ると作成する)しそのリクエストを処理するスレッドのスレッドローカルに保持される.
						その SecurityContext は SecurityContextHolder を通じて取得することができる.
						認証成功時に作成された Authentication オブジェクトはSecurityContext に保持されているため,
						これもまたSecurityContextHolder を通じて取得することができる.
