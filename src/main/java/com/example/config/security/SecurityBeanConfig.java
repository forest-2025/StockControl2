...
認証成功 or 失敗			認証成功 → authenticated=true の Authentication オブジェクトを返す.
						...
						このオブジェクトは SecurityContextPersistenceFilter が HTTP セッションに保存.
						SpringSecurity の SecurityContextPersistenceFilter が SecurityContextHolder.getContext()
						を呼び出し,現在のスレッドの SecurityContext を取得する.生成された認証済み Authentication オブジェクトを,
						...
						SecurityContextPersistenceFilter はリクエストが開始すると呼ばれ,HTTPセッションから SecurityContext を復元,
						なければ新規作成し, SecurityContextHolder（ThreadLocal） にセットする.
